...
- **manual** — live until you fire `capture`; the grabbed frame then holds
  (over `wet`) until `release`. The performance / freeze-on-a-hit mode.
...
|---|---|---|
| `mode` | manual / stutter | See above. Structural — not modulated. |
| `holdTime` | 20–2000 ms | stutter: how long each captured frame holds before re-grabbing. Short = fast judder; long = chunky freeze. Bind to tempo. *audio-bindable* |
...
live). In **stutter** the fade is **capped to the hold window** so each held
frame fully fades before the next grab — otherwise a long `fadeTime` re-grabs
before fading and you'd see no fade at all.
...
| `fadeTime` | 50–5000 ms | How long the fade takes after a freeze (ignored for `off`). Full duration in manual; capped to `holdTime` in stutter. *audio-bindable* |
...
|---|---|---|
| `capture` | — | Grab the current frame and hold it. In **manual** mode this freezes the screen until `release`; in **stutter** it re-syncs the window to now (re-grab on the beat). The snapshot-grab reaction. |
| `release` | — | Resume live in manual mode (unfreeze). No-op in stutter. |
...
`holdTime`, `dry`, `wet`, `fadeTime` and `flickerRate` carry the cross-host
...
resolved values via `setModulatedValues()`. Harness reads
...
> **mode-specific** — `holdTime` only matters in stutter, `wet` only when
> something is frozen. In **manual mode with nothing captured the filter is a
> passthrough**, so bindings have nothing to act on. (2) A base value at the
> rail has no headroom — `wet`/`dry` default to 1, so a *positive* binding is
> clamped; bind **bipolar**, or lower the base. **`dry` is the always-active
> target**: it works in every mode whether or not anything is frozen — bind a
> level there first to confirm audio is flowing.

Good starting bindings: **tempo → `holdTime`** (beat-locked stutter),
**peak → `dry`** (the original dims on hits while the freeze hangs),
**bass → `wet`** (the freeze deepens with the low end).
...
- **Beat stutter:** `mode` stutter, bind `holdTime` to tempo, `wet` 1.
...
held overlay is verified visually in a host.
