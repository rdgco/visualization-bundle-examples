...
export const description = 'Procedural night-time city skyline. Hundreds of buildings with a configurable mix of footprints (boxes, beveled/chopped corners, rare L-shapes and round towers) and window facades (standard punched, small-gap, and full-glass curtain walls), with lit windows whose glow can fill the pane or sit inset within it, rooftop features, red aviation lights blinking on the tallest spires, and a ground plane that can switch from classic glow pools to paved streets derived from the building layout — asphalt with lane markings, sidewalks, crosswalks, and warm streetlights where the city is, greenspace in the open areas between. Traffic runs as GPU-animated 3D vehicle bodies (cars and box trucks) with headlight pools cast on the road, stopping at red signals and flowing on green. Pattern variety layers in real facade types — mullioned curtain walls, ribbon and vertical-strip windows, alternating spandrel floors — hashed per face and split into floor-lit offices and scatter-lit residential. Built on three GLSL shader programs sharing one canvas; driven by the harness orbit camera. Demonstrates the full WebGL contract surface, all param kinds, the `pulse` reaction with three entry strategies, and `wantsCamera` opt-in.';
